...
def render_game_screen():
    ...
    if _g()["confirm_exit"]:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if st.button("Ja, avsluta"):
                st.session_state.game = _default_state()
                st.rerun()
        with c2:
            if st.button("Nej, fortsätt"):
                _g()["confirm_exit"] = False
                st.rerun()
        return
    ...
    with btn_col:
        ...
        if st.button("Kasta", disabled=roll_disabled):
            if _g()["roll_count"] >= 3:
    ...
    if _g()["roll_count"] == 0:
        st.write("Kasta först för att se poängförslag.")
    ...
    opt_col, score_col = st.columns([2, 1])
    with opt_col:
        if category_options:
    ...
    with score_col:
        ...
        if st.button("Registrera", disabled=register_disabled):
            if _g()["roll_count"] == 0:
